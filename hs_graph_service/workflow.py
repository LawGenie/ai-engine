...
async def vector_search_node(state: WorkflowState) -> WorkflowState:
    """벡터 검색으로 Top-5 후보 찾기 (화장품/식품 특화 쿼리)"""
    ...
    # 더 많은 후보를 검색한 후 필터링 (5개 → 최종 3개)
    candidates = await asyncio.to_thread(
        vector_service.search_similar, enhanced_query, top_k=5
    )
    ...
    for i, candidate in enumerate(candidates):
        ...
        # 상세 로깅
        logger.info(f"  #{i+1}: {candidate['hts_number']} (score: {candidate['similarity']:.3f}, tariff: {candidate['final_rate_for_korea']}%)")
...
async def finalize_node(state: WorkflowState) -> WorkflowState:
    ...
    # 최고 정확도 Top 3만 선별
    state["suggestions"] = state["suggestions"][:3]
    
    elapsed_time = time.time() - start_time
    logger.info(f"✅ Finalized Top 3 results in {elapsed_time:.3f}s")
    
    # 최종 결과 로깅 (계층적 설명 포함)
    for i, s in enumerate(state["suggestions"]):
        logger.info(f"  🏆 #{i+1}: {s['hsCode']} (confidence: {s['confidenceScore']:.3f}, tariff: {s['usTariffRate']}%)")
